  ...
  List<charts.Series<ShotState, double>> _createData(FluTheme theme) {
    return [
      charts.Series<ShotState, double>(
        ...
        colorFn: (_, __) => charts.ColorUtil.fromDartColor(theme.backgroundColor),
      ...
      charts.Series<ShotState, double>(
        ...
        colorFn: (_, __) => charts.ColorUtil.fromDartColor(theme.secondaryColor),
        strokeWidthPxFn: (_, __) => 3,
        data: dataPoints,
      ),
    ];
      ...
      child: charts.LineChart(
        ...
        behaviors: [
          ...
          charts.RangeAnnotation([
            charts.RangeAnnotationSegment(9.5, 12, charts.RangeAnnotationAxisType.measure,
        ...
        primaryMeasureAxis: charts.NumericAxisSpec(
          renderSpec: charts.GridlineRendererSpec(
            labelStyle: charts.TextStyleSpec(fontSize: 10, color: charts.ColorUtil.fromDartColor(theme.primaryColor)),
            lineStyle: charts.LineStyleSpec(thickness: 0, color: charts.ColorUtil.fromDartColor(theme.primaryColor)),
          ),
        ),
        ...
        children: [
          Row(
            children: [
              ...
              Text(machineService.state.coffeeState.toString().substring(12).toUpperCase(),
          ...
          Row(
            children: [
              ...
              Text(machineService.state.shot.groupPressure.toStringAsFixed(1) + ' bar', style: theme.tabPrimaryStyle),
            ],
          ),
          Row(
            children: [
              Text('Flow: ', style: theme.tabSecondaryStyle),
              Text(machineService.state.shot.groupFlow.toStringAsFixed(2) + ' ml/s', style: theme.tabPrimaryStyle),
            ],
          ),
          Row(
            children: [
              Text('Mix Temp: ', style: theme.tabSecondaryStyle),
              Text(machineService.state.shot.mixTemp.toStringAsFixed(2) + ' °C', style: theme.tabPrimaryStyle),
            ],
          ),
          Row(
            children: [
              Text('Head Temp: ', style: theme.tabSecondaryStyle),
              Text(machineService.state.shot.headTemp.toStringAsFixed(2) + ' °C', style: theme.tabPrimaryStyle),
            ],
          ),
        ],
      );
    } else {
      insights = Text('Machine is not connected', style: theme.tabPrimaryStyle);
    }
    return insights;
  }
              ...
              children: <Widget>[
                Row(
                  children: [
                    ...
                    Text(snapshot.data.weight.toStringAsFixed(2) + 'g', style: theme.tabSecondaryStyle),
                  ],
                ),
                Row(
                  children: [
                    Text('Flow: ', style: theme.tabSecondaryStyle),
                    Text(snapshot.data.flow.toStringAsFixed(2) + 'g/s', style: theme.tabSecondaryStyle)
                  ],
                ),
              ],
            );
      ...
      child: Scaffold(
            ...
            children: [
              ...
              Flexible(
                child: RaisedButton(
                  ...
                  color: pressAttention ? theme.goodColor : theme.badColor,
        ...
        body: TabBarView(
          children: [
            Container(
              decoration: BoxDecoration(
                gradient: theme.screenBackground,
              ),
              child: Column(
                children: <Widget>[
                  _buildLiveInsights(theme),
                  theme.horizontalBorder(),
